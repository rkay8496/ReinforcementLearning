            ...
            for k in converted:
                for l in k:
                    if l['is_dummy'] and j['is_dummy']:
                        l['No'] = j['No']
                    elif l['speed'] == j['speed'] and l['distance'] == j['distance'] and \
                            l['action'] == j['action']:
                        l['No'] = j['No']
    return converted
...
def generate_prism_model(state_list):
    ...
    for state in state_list:
        data += '\t[] '
        data += '(speed = ' + str(state['speed']) + ') & (distance = ' + str(state['distance']) + \
                ') & (action = ' + str(state['action']) + ') -> '
        if len(state['next']) > 0:
            for next in state['next']:
                data += str(next['probability']) + ':(speed\' = ' + str(next['speed']) + ') & (distance\' = ' + str(next['distance']) + \
                        ') & (action\' = ' + str(next['action']) + ') + '
            data = data[:-3]
            data += ';\n'
        else:
            data += '1:(speed\' = ' + str(state['speed']) + ') & (distance\' = ' + str(state['distance']) +  \
                    ') & (action\' = ' + str(state['action']) + ')'
            data += ';\n'
    data += 'endmodule\n'
    data += '\n'
    ...
    for state in state_list:
        if not state['is_dummy']:
            data += 'label \"s' + str(state['No']) + '\" = ' + '(speed = ' + str(state['speed']) + ') & (distance = ' + str(state['distance']) + \
                ') & (action = ' + str(state['action']) + ');\n'
    for state in state_list:
        if state['is_dummy']:
            data += 'label \"dummy' + str(state['No']) + '\" = ' + '(speed = ' + str(state['speed']) + ') & (distance = ' + str(state['distance']) + \
                ') & (action = ' + str(state['action']) + ');\n'
    data += '\n'
    data += 'rewards "step"\n'
    data += '\t[] true : 1;\n'
